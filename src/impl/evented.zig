...
/// B (k, j)
/// C (i, j)
pub fn evented(comptime T: type, _: anytype, c: *Matrix(T), a: Matrix(T), b: Matrix(T)) !void {
    ...
    var evented_io: std.Io.Evented = undefined;
    var arena_allocator = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena_allocator.deinit();
    const allocator = arena_allocator.allocator();
    try evented_io.init(allocator, .{});
    ...
    switch (c.major) {
        ...
        .ColumnMajor => {
            if (a.major == .ColumnMajor) {
                for (0..nj) |j| {
                    try group.concurrent(io, W.ki, .{ ni, j, nk, c, a, b });
                }
                // if b doesn't match, well, nothing better to do
            } else {
                for (0..nj) |j| {
                    try group.concurrent(io, W.ik, .{ ni, j, nk, c, a, b });
                }
            }
        },
    }
...
test evented {
    ...
    try evented(u32, std.testing.io, &c, a, b);
